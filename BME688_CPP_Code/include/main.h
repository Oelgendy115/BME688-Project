...
//---------------------------------------------------------------------
// Wrapper for the native heater configuration (bme68x_heatr_conf)
// We wrap it with an ID for mapping from the configuration file.
struct HeaterConfig {
  String id;                      // Identifier from the config file
  bme68x_heatr_conf conf;         // Native heater configuration structure
};

// Duty cycle structures remain the same.
...
// New: Sensor configuration structure loaded from the config file.
struct SensorConfig {
  uint8_t sensorIndex;
...
extern HeaterConfig heaterConfigs[4];  // Array of heater configurations
...
// Function Prototypes
void errLeds(void);
bool setHeaterProfile(uint8_t profileIndex, Bme68x &sensor);  // Wrapper function that returns bool
void getHeaterProfiles();
void initializeHeaterConfigs(); // Hardcoded heater configuration assignment
...
void loadDynamicConfig();  // SD card–based configuration loader

// Sensor assignment functions
void assignDynamicSensorConfigs();
void assignHardcodedSensorConfigs();

// New function to print duty cycle assignments
